...
The small intervals constructed are equivalent due the fact that multiplying the interval by $q^{-1}$, preserves or swaps the original cardinality of quadratic residues and non-residues, since $\chi_2((N+k)q^{-1})=\chi_2(q^{-1})\chi_2(N+k)$, and $\chi_2(q^{-1})$ is a constant. We now prove an important lemma which solidifies the effect of the amplification.
%
\begin{lemma}
